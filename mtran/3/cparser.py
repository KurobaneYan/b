...
def check(filename):
    p1 = Popen(['gcc', filename], stderr=PIPE)
    p2 = Popen(['grep', 'error'], stdin=p1.stderr, stdout=PIPE)
    p1.stderr.close()
    output = p2.communicate()[0]
    if output != '':
        error = output.replace('\n', ':').split(':')
        print >>sys.stderr, "Parse Error at Line %s / Char %s - %s" % (error[1], error[2], error[4].strip())
        sys.exit()
...
def parse_declaration( tokens ):
    ...
    while len(tokens):
        ...
        if name in current_scope:
            print ('Parse Error at Line %d / Char %d - redeclaration of "{}"'.format(name.line, name.pos, name))
            raise Error()

        variables[name] = type[1]
...
def check_semantics(tokens):
    id_table = build_id_table(tokens)
    result = True

    for key, value in id_table.items():
        if key not in ['printf', 'argv', 'argc']:
            if value[0] == 'UNKNOWN':
                token = [t for t in tokens if t.value == key][0]
                print 'Semantic error: undeclared identifier "{}" at ({}:{})'.format(key, token.line, token.pos)
                result = False

    return result and check_functions(_map(tokens))


def check_function_call(name, args):
    if name in declared_functions:
        func_args = declared_functions[name]

        if len(func_args) > len(args):
            print 'Semantic error: too few arguments to function "calc_factorial" at ({}:{})'.format(name.line, name.pos)
        elif len(func_args) < len(args):
            print 'Semantic error: too many arguments to function "calc_factorial" at ({}:{})'.format(name.line, name.pos)

        for declared_arg, arg in zip(args, func_args):
            if declared_arg != arg:
                return False
        return True
    else:
...
def parse_block( tokens ):
    ...
    variables = variables['outer_scope']
...
def print_thing( thing, depth=0 ):
    ...
    if name == "Block":
        ...
        for num,statement in enumerate(value):
            print TAB*(depth)+ "Statement %d" %(num+1)
    ...
    for filename in files:
        ...
        if options.tokens:
            ...
            for i,token in enumerate( tokens ):
                ...
                loc = ("[%d, %d]: " %(token.line, token.pos)).ljust(10)
                print loc,token
