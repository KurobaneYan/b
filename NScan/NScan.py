...
import getopt,sys,Queue,threading,socket,struct,urllib2,time,os,re,json,cgi,array,ssl
        ...
        while True:
            try:
                ...
                if ac_ip not in recvFroms:
                    hostName = socket.gethostbyaddr(ac_ip)[0]
                    log("active", ac_ip, 0, hostName)
                    recvFroms.add(ac_ip)
            except Exception:
                pass
            finally:
            ...
            try:
                ...
                if data:
                    ...
                    if not server_type:
                        ...
                        if title or h_server:
                            server_type = 'web ' + title
                    if server_type:
                        log('server',task_host,task_port,server_type.strip())
            except Exception,e:
                continue
...
def log(scan_type,host,port,info=''):
    ...
    try:
        ...
        if scan_type == 'portscan':
            ...
            print "[%s] %s (hostname: %s) is active"%(time_str, host, info)
    except Exception,e:
        pass
    mutex.release()
def read_config(config_type):
    if config_type == 'server_info':
        mark_list=[]
        try:
            config_file = open('settings.ini','r')
...
if __name__=="__main__":
    ...
    msg = '''
Scanning a network asset information
Usage: python NScan.py -h 192.168.1 [-p 21,80,3306] [-m 50] [-n]
    '''
    if len(sys.argv) < 2:
        print msg
    try:
